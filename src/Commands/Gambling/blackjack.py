...
`hit` or `stand`""", title="Blackjack")
    ...
    def msg_check(m):
        ...
        if m.content.lower() != "hit" and m.content.lower() != "stand":
            return False
    ...
    while True:
        try:
            ...
            await message.edit_message(msg, sent_msg.embeds[0].description, title="Timed out")
            return

        userinfo = user.load_user(msg.author.id, conn)
        user_msg.content = user_msg.content.lower()

        if user_msg.content == "hit":
            random_card = random.choice(deck)
            user_cards.append(random_card)
            deck.remove(random_card)

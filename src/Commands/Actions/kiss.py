...
gif_url = "https://raw.githubusercontent.com/Talia-Team/Talia-Assets/main/actiongifs/kiss"
...
async def run(bot, msg, conn):
    ...
    if person_id == msg.author.id:
        await message.send_error(msg, random.choice(self_kiss))
        return
    else:
        try:
    ...
    random_image = f"{gif_url}/kiss{random.randint(1, 18)}.gif"
    await message.send_message(msg, title=f"{str(msg.author)} kissed {str(person)}{random.choice(suffix)}",
        img=random_image
    )